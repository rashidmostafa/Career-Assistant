...
function getEmailState(email: string) {
  ...
  if (!EMAIL_REGEX.test(email)) return { valid: false, hint: "Please enter a valid email (e.g., name@domain.com)", typoSuggestion: null };
...
export default function AuthScreen() {
  ...
  // If pending verification, show the verification screen
  if (pendingVerificationEmail) {
    return (
      ...
      >
        <ScrollView contentContainerStyle={[styles.scroll, { paddingTop: topPad + 60, paddingBottom: insets.bottom + 40 }]} keyboardShouldPersistTaps="handled">
          <Animated.View entering={FadeInDown.duration(600).springify()} style={styles.header}>
            ...
            <Text style={[styles.tagline, { color: colors.mutedForeground, textAlign: "center" }]}>
              ...
              <Text style={{ fontFamily: "Inter_700Bold", color: colors.foreground }}>{pendingVerificationEmail}</Text>
            </Text>
          </Animated.View>

          <Animated.View entering={FadeInUp.duration(800).springify()} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
  ...
  const handleSubmit = async () => {
    ...
    if (!emailState.valid) { setError("Please enter a valid email address."); return; }
    ...
    try {
      if (mode === "login") {
        ...
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        // Will show verification screen automatically since pendingVerificationEmail is set
      }
    } catch (e: any) {
  ...
  const SocialButton = ({ icon, label, onPress }: { icon: string; label: string; onPress: () => void }) => (
    <TouchableOpacity
  ...
  return (
    ...
    >
      <ScrollView
        contentContainerStyle={[styles.scroll, { paddingTop: topPad + 60, paddingBottom: insets.bottom + 40 }]}
      ...
      >
        <Animated.View entering={FadeInDown.duration(600).springify()} style={styles.header}>
          ...
          <Text style={[styles.tagline, { color: colors.mutedForeground }]}>Your intelligent career assistant</Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeInUp.duration(800).springify()} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <View style={[styles.tabRow, { backgroundColor: colors.muted }]}>
            {(["login", "register"] as const).map((m) => (
              <TouchableOpacity
                ...
                onPress={() => { setMode(m); setError(""); setEmailTouched(false); }}
              >
                <Text style={[styles.tabText, { color: m === mode ? colors.foreground : colors.mutedForeground }]}>
          ...
          <View style={styles.fieldGroup}>
            ...
            <View style={[styles.inputRow, { backgroundColor: colors.background, borderColor: emailBorderColor }]}>
              ...
              <TextInput
                ...
                placeholderTextColor={colors.mutedForeground}
                value={email}
                onChangeText={(t) => { setEmail(t); setEmailTouched(false); }}
            ...
            {emailState.typoSuggestion && emailState.valid && (
              <TouchableOpacity
                style={[styles.typoHint, { backgroundColor: colors.warning + "15", borderColor: colors.warning + "40" }]}
          ...
          <View style={styles.fieldGroup}>
            ...
            {mode === "register" && password.length > 0 && password.length < 6 && (
              <Text style={[styles.fieldHint, { color: colors.destructive }]}>Password must be at least 6 characters</Text>
            )}
          </View>
          ...
          <View style={styles.socialGroup}>
            ...
            <SocialButton
              ...
              onPress={() => {
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                setError(''); // Email form is always visible above
              }}
            />
            <SocialButton
              icon="···"
...
const styles = StyleSheet.create({
  ...
  logoWrap: { width: 64, height: 64, borderRadius: 20, alignItems: "center", justifyContent: "center", marginBottom: 16, shadowColor: "#2563eb", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 8 },
  appName: { fontSize: 32, fontFamily: "Inter_700Bold", letterSpacing: -0.5 },
  tagline: { fontSize: 15, fontFamily: "Inter_500Medium", marginTop: 6 },
  card: { borderRadius: 24, padding: 24, borderWidth: 1, shadowColor: "#000", shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.05, shadowRadius: 24, elevation: 4 },
  ...
  socialBtn: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 14, paddingHorizontal: 18, borderRadius: 12, borderWidth: 1 },
  ...
  input: { borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, fontSize: 15, fontFamily: "Inter_500Medium", borderWidth: 1 },
  inputRow: { flexDirection: "row", alignItems: "center", borderRadius: 12, paddingHorizontal: 16, paddingVertical: 14, borderWidth: 1, gap: 8 },
  inputInner: { flex: 1, fontSize: 15, fontFamily: "Inter_500Medium" },
  errorBox: { flexDirection: "row", alignItems: "center", gap: 8, padding: 12, borderRadius: 10, marginBottom: 20, borderWidth: 1 },
  ...
  typoHint: { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 8, padding: 10, borderRadius: 10, borderWidth: 1 },
  typoText: { fontSize: 13, fontFamily: "Inter_500Medium", flex: 1 },
  btn: { paddingVertical: 16, borderRadius: 12, alignItems: "center", marginTop: 8, shadowColor: "#2563eb", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
  ...
  verifyDesc: { fontSize: 15, fontFamily: "Inter_500Medium", lineHeight: 22, marginBottom: 20, textAlign: "center" },
  verifyBox: { flexDirection: "row", alignItems: "flex-start", gap: 10, padding: 14, borderRadius: 12, borderWidth: 1 },
